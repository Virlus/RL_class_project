...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim, dist_dim, init_w=1e-3):
        ...
        self.q_net = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim), nn.LeakyReLU())
        self.q_last = nn.Linear(hidden_dim, dist_dim)

    def forward(self, obs, action):
        obs_action = torch.cat([obs, action], dim=-1)
        q = self.q_net(obs_action)
        q = self.q_last(q)
        return q


class DIST_CDSAgent(Agent):
    def __init__(self,
                 name,
                 obs_shape,
                 action_shape,
    ...
    def _repeated_critic_apply(self, obs, actions):
        ...
        Q = self.critic(repeated_obs, reshaped_actions)
        Q = Q.reshape((n_samples, batch_size, -1))
        Q = F.softmax(Q, dim=-1)
        Q = torch.sum(Q * self.q_grid, dim=-1, keepdim=True)
        return Q
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        critic_loss = F.cross_entropy(Q, target_Q_dist, reduction='mean')
        ...
        # Add CQL penalty
        with torch.no_grad():
            random_actions = torch.FloatTensor(self.n_samples, Q.shape[0],
        ...
        rand_Q = self._repeated_critic_apply(obs, random_actions)  # (n_samples, 1024, 1)
        sampled_Q = self._repeated_critic_apply(                # (n_samples, 1024, 1)
            obs, sampled_actions)
        next_sampled_Q = self._repeated_critic_apply(      # (n_samples, 1024, 1)
            obs, next_sampled_actions)

        cat_Q = torch.cat([rand_Q, sampled_Q, next_sampled_Q,
                            Q.unsqueeze(0)], dim=0)                # (1+3*n_samples, 1024, 1)

        assert (not torch.isnan(cat_Q).any())
        
        cql_logsumexp = torch.logsumexp(cat_Q, dim=0).mean()

        cql_penalty = cql_logsumexp - Q.mean()
        ...
        if self.use_tb:
            ...
            metrics['rand_Q'] = rand_Q.mean().item()
            metrics['sampled_Q'] = sampled_Q.mean().item()
    ...
    def update_actor(self, obs, action, step):
        ...
        # optimize actor
        Q = self.critic(obs, sampled_action)
        Q = F.softmax(Q, dim=-1)
        Q = torch.sum(Q * self.q_grid, dim=-1, keepdim=True)
    ...
    def conservative_data_share(self, batch_main, batch_share):
        ...
        with torch.no_grad():
            conservative_q_dist = self.critic(obs, action)
            conservative_q_dist = F.softmax(conservative_q_dist, dim=-1)
            conservative_q_value = torch.sum(conservative_q_dist * self.q_grid, dim=-1, keepdim=True)
