...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim, dist_dim, init_w=1e-3):
        super().__init__()
        ...
        self.q1_net = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim), nn.LeakyReLU())
        self.q1_last = nn.Linear(hidden_dim, dist_dim)

        self.q2_net = nn.Sequential(
            nn.Linear(obs_dim + action_dim, hidden_dim), nn.LayerNorm(hidden_dim), nn.Tanh(),
            nn.Linear(hidden_dim, hidden_dim), nn.LayerNorm(hidden_dim), nn.LeakyReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.LeakyReLU())
        self.q2_last = nn.Linear(hidden_dim, dist_dim)
...
class DDRCDSAgent(Agent):
    def __init__(self,
        ...
        self.q_grid = torch.linspace(q_lower, q_upper, dist_dim, dtype=torch.float32).to(device)
        ...
        self.critic = Critic(state_dim, action_dim, hidden_dim, dist_dim).to(device)
        self.critic_target = Critic(state_dim, action_dim, hidden_dim, dist_dim).to(device)
    ...
    def _repeated_critic_apply(self, obs, actions):
        ...
        Q1, Q2 = F.softmax(Q1, dim=-1), F.softmax(Q2, dim=-1)
        Q1 = torch.sum(Q1 * self.q_grid, dim=-1, keepdim=True)
        Q2 = torch.sum(Q2 * self.q_grid, dim=-1, keepdim=True)

        return Q1, Q2
    
    def _target_critic_apply(self, next_obs, sampled_next_action, next_action_log_prob, discount, reward):
        with torch.no_grad():
            alpha = self.log_actor_alpha.exp().detach()
            target_Q1, target_Q2 = self.critic_target(next_obs, sampled_next_action) # (1024, self.dist_dim)
            target_Q1, target_Q2 = F.softmax(target_Q1, dim=-1), F.softmax(target_Q2, dim=-1)
            fixed_q_grid = self.q_grid.unsqueeze(0).repeat(target_Q1.shape[0], 1)  # (1024, self.dist_dim)
            ...
            floor_values = self.q_grid[floor_indices]
            ceil_values = self.q_grid[ceil_indices]
            floor_weights = (ceil_values - target_q_grid) / (ceil_values - floor_values).clip(min=1e-6)
            ceil_weights = 1. - floor_weights
            target_Q = 0.5 * target_Q1 + 0.5 * target_Q2
            target_Q_dist = torch.zeros_like(target_Q, device = self.device)
            target_Q_dist.scatter_add_(1, floor_indices, target_Q * floor_weights)
            target_Q_dist.scatter_add_(1, ceil_indices, target_Q * ceil_weights)
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            ...
            target_Q_dist = self._target_critic_apply(next_obs, sampled_next_action, next_action_log_prob, discount, reward) # (1024, self.dist_dim)

        Q1, Q2 = self.critic(obs, action)
        # Q1_weighted = Q1 * weights.unsqueeze(-1)
        # Q2_weighted = Q2 * weights.unsqueeze(-1)
        # target_Q_weighted = target_Q * weights.unsqueeze(-1)
        critic_loss = F.cross_entropy(Q1, target_Q_dist, reduction='mean') + F.cross_entropy(Q2, target_Q_dist, reduction='mean')
    ...
    def update_actor(self, obs, action, step):
        ...
        # optimize actor
        Q1, Q2 = self.critic(obs, sampled_action)     # (1024, dist_dim)
        Q1, Q2 = F.softmax(Q1, dim=-1), F.softmax(Q2, dim=-1)
        Q1, Q2 = torch.sum(Q1 * self.q_grid, dim=-1, keepdim=True), torch.sum(Q2 * self.q_grid, dim=-1, keepdim=True)
        Q = torch.min(Q1, Q2)
    ...
    def conservative_data_share(self, batch_main, batch_share):
        ...
        with torch.no_grad():
            ...
            conservative_q_value_dist = F.softmax(conservative_q_value, dim=-1)
            conservative_q_value = torch.sum(conservative_q_value_dist * self.q_grid, dim=-1, keepdim=True)
